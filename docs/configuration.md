...
去重算法在咨询类分支的循环检索中应用，用于去除重复片段，提升结果多样性和质量。

### 配置位置

去重参数主要在以下位置配置：

1. **轮内去重**：在 `src/graphs/loop_graph.py` 中硬编码（可提取为配置文件）
2. **轮间去重**：在 `src/graphs/nodes/consult/consult_results_consolidation.py` 中硬编码（可提取为配置文件）

### 咨询类去重参数

#### 轮内去重参数

**配置位置**：`src/graphs/loop_graph.py` 中的 `consult_retrieval_internal_node`

| 参数 | 值 | 说明 |
|------|-----|------|
| `similarity_threshold` | 0.70 | Jaccard 相似度阈值，用于贪心聚类 |
| `mmr_lambda` | 0.85 | MMR 相关性权重（0-1之间） |
| `mmr_top_k` | 10 | MMR 重排序后保留的片段数 |
...
**配置位置**：`src/graphs/nodes/consult/consult_results_consolidation.py` 中的 `consult_results_consolidation_node`

| 参数 | 值 | 说明 |
|------|-----|------|
| `similarity_threshold` | 0.75 | Jaccard 相似度阈值，用于轮间去重 |
| `quality_filter_threshold` | 0.3 | 质量过滤阈值，低于此值的片段被过滤 |
| `final_top_k` | 15 | 最终保留的片段数 |
...
- **similarity_threshold（相似度阈值）**：
  - 值域：[0, 1]
  - 值越高，去重越严格（更多片段被判定为重复）
  - 值越低，去重越宽松（更少片段被判定为重复）
  - 建议：0.70-0.80
  - 比轮内阈值稍高，减少误删

- **quality_filter_threshold（质量过滤阈值）**：
  - 值域：[0, 1]
  - 值越高，过滤越严格（更多低分片段被过滤）
  - 值越低，过滤越宽松（更多低分片段被保留）
  - 建议：0.25-0.35
...
**配置位置**：`src/graphs/nodes/judge/judge_retrieval.py` 中的 `judge_retrieval_node`

| 参数 | 值 | 说明 |
|------|-----|------|
| `similarity_threshold` | 0.70 | Jaccard 相似度阈值，用于贪心聚类 |
| `mmr_lambda` | 0.88 | MMR 相关性权重（0-1之间） |
| `mmr_top_k` | 12 | MMR 重排序后保留的片段数 |
| `paragraph_similarity_threshold` | 0.75 | 段落级别去重阈值 |

### 去重参数对比

| 参数 | 咨询类（轮内） | 咨询类（轮间） | 行为判断类 |
|------|---------------|---------------|-----------|
| Jaccard 阈值 | 0.70 | 0.75 | 0.70 |
| MMR lambda | 0.85 | - | 0.88 |
| MMR top_k | 10 | - | 12 |
| 质量过滤阈值 | - | 0.3 | - |
| 最终 top_k | - | 15 | 12 |
...
- **咨询类更重视多样性**：
  - MMR lambda 较低（0.85 vs 0.88）
  - 保留更多片段（15 vs 12）
  - 需要覆盖问题的多个方面

- **行为判断类更重视精确性**：
  - MMR lambda 较高（0.88 vs 0.85）
  - 保留更少片段（12 vs 15）
  - 需要精确匹配行为和规则
...
```python
# 提高相似度阈值
similarity_threshold = 0.80  # 从0.75提高到0.80

# 或者提高MMR的多样性权重
mmr_lambda = 0.80  # 从0.85降低到0.80
```
...
```python
# 降低相似度阈值
similarity_threshold = 0.70  # 从0.75降低到0.70

# 或者提高MMR的相关性权重
mmr_lambda = 0.90  # 从0.85提高到0.90

# 或者增加保留片段数
mmr_top_k = 12  # 从10增加到12
final_top_k = 20  # 从15增加到20
```
...
```python
# 提高质量过滤阈值
quality_filter_threshold = 0.4  # 从0.3提高到0.4

# 或者调整检索参数
min_score = 0.4  # 从0.3提高到0.4
```
...
1. **批量处理**：对大量片段进行去重时，使用批量处理减少循环次数
2. **早期截断**：在分数较低时提前截断，减少计算量
3. **缓存优化**：缓存Jaccard相似度计算结果，避免重复计算

### 未来改进方向

1. **配置文件化**：将硬编码参数提取为配置文件
2. **自适应阈值**：根据检索结果质量动态调整阈值
3. **语义相似度**：使用嵌入模型替代Jaccard，提升相似度计算精度
4. **多层级去重**：引入段落级别、文档级别的多层级去重
