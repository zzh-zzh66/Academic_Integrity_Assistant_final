...
- **分层去重策略**：轮内去重（贪心聚类+MMR）+ 轮间去重（Jaccard）
- **贪心聚类算法**：使用Jaccard相似度（threshold=0.70）对检索结果进行聚类
- **代表片段选择**：每个聚类保留最高分片段，确保质量
- **MMR重排序**：平衡相关性与多样性（lambda=0.85, top_k=10）
- **轮间去重**：合并历史和当前结果，去除重复片段（Jaccard, threshold=0.75）
- **结果整合**：质量过滤（score >= 0.3）、保留top-15、计算评估指标
- **总体去重率**：约60%，大幅提升结果多样性和实用性

### 行为判断类去重优化

- **贪心聚类算法**：使用Jaccard相似度（threshold=0.70）对检索结果进行聚类
- **代表片段选择**：每个聚类保留最高分片段，确保质量
- **MMR重排序**：平衡相关性与多样性（lambda=0.88, top_k=12）
- **段落级别去重**：按doc_id分组去重（Jaccard, threshold=0.75），保证文档来源多样性
- **智能聚合**：合并相似规则，避免重复表述，提升判断全面性
...
首创分层去重策略，大幅提升检索结果的多样性和实用性：
- **轮内去重**：每轮检索后立即执行，贪心聚类（Jaccard 0.70）+ MMR重排序（lambda 0.85）
- **轮间去重**：多轮结果合并时执行，基于Jaccard相似度（0.75）去除重复
- **结果整合**：质量过滤、Top-K截断、评估指标计算，生成统一输出
- **去重效果**：总去重率约60%，性能开销<5%
